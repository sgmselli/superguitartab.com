[Superguitartab.com](../README.md) >
Developer documentation

# Developer documentation

The full developer documentation is organized into separate sections for clarity. Use the links below to navigate:

### Architecture
- [System Overview](./architecture/system-overview.md)

### Setup & Configuration
- [Local Development Setup](./setup/setup.md)

### API
- [API Reference](./api/api-reference.md)
- [Database Models (SQLAlchemy)](./api/models.md)

### Deployments
- [Our environments](./deployments/our-environments.md)
- [CI/CD (GitHub Actions)](./deployments/ci-cd.md)
- [Infrastructure Provisioning](./deployments/provisioning.md)
- [Infrastructure Used](./deployments/infrastructure.md)

### Database
- [Database](./database/database.md)

### Testing
- [Testing](./testing/testing.md)

### Monitoring
- [Monitoring](./monitoring/monitoring.md)